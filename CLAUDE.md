...
- [ ] **Calendar fills available height** — see `calendar-fills-height` task. Investigation-heavy; previous attempts at flex/100% didn't move the needle, EC seems to internally cap row height. Next angle: read EC's source for month-view sizing logic, look for a `--ec-day-height` token.

### Phase 5 — Keyboard + notification UX
- [ ] **Quick-create shortcuts** (`quick-create-shortcuts` task). `Cmd+N` new card in active column, `Cmd+Shift+N` new note, `Cmd+Alt+N` new board. Reuse the existing inline-create flow.
- [ ] **Click-notification → focus task** (`click-notification-focus-task` task). Wire `tauri-plugin-notification` action API; payload includes the task path, click handler sets `ui.openTaskPath`.
