...
- [x] **Terminal glyph coverage.** Bundled JetBrains Mono Nerd Font Mono (Regular + Bold only, ~4.7MB total) in `static/fonts/`. `@font-face` declarations + `--font-mono` token override in `app.css` so Tailwind's `font-mono` utility points at it; an explicit rule forces `.cm-editor` and `.prose code/pre` onto the same family. `Terminal.svelte` awaits `document.fonts.load` for both weights before constructing xterm. Italic/bold-italic synthesized from Regular by the browser (rare in terminal/code contexts). Required a separate fix in `pty.rs` (`drain_valid_utf8`) so multi-byte UTF-8 sequences split across PTY read boundaries don't get replaced with U+FFFD before reaching xterm — see commit b73a2e8.
- [x] **Persistent terminal session.** `<Terminal />` is mounted on first open (lazy via a `terminalEverOpened` flag) and kept mounted thereafter; toggling Cmd+J just flips a `class:hidden` on the section, so the PTY survives panel close. xterm's existing `ResizeObserver` re-fits when the section becomes visible. Session still ends on app quit (PTY child process is killed by `Terminal.svelte` `onDestroy` and reaped by the OS). Same change moved the panel inside the right-of-sidebar column so it no longer bleeds under the sidebar.

### Phase 4 — Search + calendar polish
- [x] **Highlight search matches** — `highlightMatches` util in `search.ts` wraps each token occurrence in `<mark class="search-hit">` (HTML-escaped, case-insensitive, regex-special-chars escaped, longer tokens preferred when overlapping). Rendered via `{@html}` in `SearchOverlay.svelte`. Translucent amber bg works on both dark and light themes.
- [x] **Search filter chips** — `FilterChip.svelte` reusable component (single + multi select). Board / Kind / Priority / Tags / Reminder chips above the search input. `searchEntries(entries, bodies, query, filters)` extended with a `SearchFilters` shape; empty query + active filter now lists everything that passes the filter. Single shared `openChipId` in the parent so opening one popup closes the others. Two non-obvious bugs surfaced building this: (1) `let open = $state(...)` shadowed `window.open` and silently no-op'd writes — renamed to `popoverOpen`; (2) Svelte 5 mounts the popup synchronously on state change, so `use:clickOutside` was registering on the same click that opened the popup and immediately closing it — fixed with `e.stopPropagation` on the chip trigger.
...
- [x] **Iter 2 — Extended scope.** GFM enabled via `markdown({ base: markdownLanguage })`. Task checkboxes via `CheckboxWidget` (CM6 transaction-based toggle, `e.stopPropagation` so the panel's clickOutside doesn't fire when the widget rebuilds). Bullet/ordered lists (raw markers, dimmed). Blockquote with left border + indent (italic suppressed). Fenced code initially with surface-2 inline-block treatment (refined in iter 3). `Tab`/`Shift+Tab` indent via `indentWithTab`. Selection background lifted to a brighter blue so it's visible on `surface-2`.
- [x] **Iter 3 — Toggle removal + look-and-feel polish.** Deleted `MarkdownPreview.svelte`, `src/lib/utils/checkbox.ts` + tests, removed `marked` + `@tailwindcss/typography` deps. Editor body now uses `--color-surface-deep` (pure black in dark) with body text pure white. Headings, blockquote, code-info language tag overridden via `.cm-editor.cm-editor` + descendant `!important` so they win against oneDark's wrapping spans. Active-line highlight set to `surface-1` to be visible on the new black bg. Fenced code refactored to per-line decorations (`Decoration.line`) with first/last classes for unified rounded box; the ``` fence delimiter lines are hidden when the cursor is outside the block.

### Phase 7 — Fenced code syntax highlighting

- [x] **Per-language syntax highlighting in fenced code blocks.** `markdown({ codeLanguages })` wired up with a curated list in `src/lib/editor/codeLanguages.ts` (filters `@codemirror/language-data`'s catalog to the languages we've installed: JS/TS, Python, JSON, HTML, CSS, SQL, YAML, shell/bash, markdown, plus a custom `LanguageDescription` for Elixir via `codemirror-lang-elixir`). Vite splits each into its own lazy-loaded chunk. Custom `HighlightStyle` (`src/lib/editor/highlight.ts` — Tokyo Night-ish dark + Catppuccin Latte-ish light) replaces oneDark's built-in token colours for higher contrast.
