...
| IPC types | `tauri-specta` (pinned RC) generates `src/lib/bindings.ts` from the Rust structs; regenerated by `cargo test`, drift-checked in CI |
| Rust errors | `thiserror` enum (`src-tauri/src/error.rs`), serialized to a string so the frontend keeps receiving plain-string rejections |
| Lint / format | Prettier + ESLint (frontend), rustfmt + `clippy -D warnings` (Rust); all enforced in CI |
...
### Phase 12 — Structural hardening

Behavior-preserving structural pass from a code-structure review (no feature changes); each item is its own commit.

- [x] **Lint / format / Clippy + CI gates.** Added Prettier (+ `prettier-plugin-svelte`), an ESLint flat config (`typescript-eslint` + `eslint-plugin-svelte`), `rustfmt.toml`, and `clippy -D warnings`, all enforced in `ci.yml` next to svelte-check/vitest/cargo test. A few opinionated svelte rules are warn/off (documented inline in `eslint.config.js`); repo-wide formatting landed as its own commit.
- [x] **Typed Rust error enum + backend modularization.** Split the 1,281-line `commands.rs` into `error.rs` / `models.rs` / `util.rs` / `commands/{vault,task,board,note,reminder,attachment,watcher}.rs`. `thiserror` `Error` serializes to a string (preserving the `DestinationExists:` prefix + every string-display site). Extracted `write_atomic` (now also used for `_silex.json` writes — previously non-atomic), a single `unique_path` collision helper, a `BoardDir` helper, and unit-tested pure watcher helpers (`event_kind_str`/`md_paths`); test count 43 → 45.
- [x] **IPC type safety via tauri-specta.** `VaultEntry`/`BoardLayout`/`VaultChange` are generated from the Rust structs into `src/lib/bindings.ts` (regenerated by `cargo test`, drift-guarded in CI). `vault.ts` is a thin adapter over the generated `commands` (`ErrorHandlingMode::Throw` → the throw/catch contract is unchanged). The runtime `generate_handler!` is untouched; `save_attachment` (raw IPC body) stays a hand-written `invoke`. Deps pinned to RC versions.
- [x] **`+layout.svelte` decomposition.** 632 → 229 lines; extracted `Sidebar.svelte`, `app/menuActions.ts`, `utils/dragToResize.ts`, and `stores/persist.ts` (`persistEntry`, deduping the byte-identical tasks/notes save-path).
...
Since Phase 12 this type is **generated** from the Rust `VaultEntry` struct into `src/lib/bindings.ts` and imported via `src/lib/api/vault.ts`, which re-narrows `frontmatter` back to `Record<string, unknown> | null`. Don't hand-edit `bindings.ts` — change the Rust struct and run `cargo test` to regenerate it.
