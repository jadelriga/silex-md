<script lang="ts">
  ...
  // `activeBoard` is a derived value owned by the layout (it reads the route);
  // passed in so the layout and the menu actions share one source of truth.
  let { activeBoard }: { activeBoard: string | null } = $props();
